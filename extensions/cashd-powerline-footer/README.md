...
**Focus-aware editor border** — The prompt border uses the theme's accent border while the input editor owns focus, switches to the bash-mode color in bash mode, and falls back to the muted border when overlays, dialogs, terminal windows, or tmux panes take focus. In tmux, enable focus forwarding with `set -g focus-events on` if pane switches do not emit focus changes.
